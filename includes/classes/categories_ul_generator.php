...
class zen_categories_ul_generator {
    ...
    function buildBranch($parent_id, $level = 1, $submenu=false) {
      ...
      if ($parent_id != '0') {
		if($level==1){
		$result.='<div class="megamenu">';
		}
        $result.= sprintf($this->parent_group_start_string, ($submenu==true) ? ' class="nav-child unstyled'. ($level) . '"' : '' );
      }
        if (($this->data[$parent_id])) {
            foreach($this->data[$parent_id] as $category_id => $category) {
              $category_link = $category_id;
			  if (($this->data[$category_id])) {
                $result .= sprintf($this->child_start_string, ($submenu==true) ? ' id="' . $category['name'] . '"' . ' class="submenu"' : '');
			  } else {
