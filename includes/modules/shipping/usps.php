 ...
 * @version $Id: usps.php 2016-01-17 ajeh Version K8 $
 */
  ...
  function __construct() { // for older php < 5.1.0 change to usps() instead of __construct()
    ...
    if (MODULE_SHIPPING_USPS_STATUS == 'True') {
      if (IS_ADMIN_FLAG) {
        ...
        if ((MODULE_SHIPPING_USPS_VERSION != '2016-01-17') || (sizeof($chk_keys) != $chk_sql->RecordCount())) {
          $this->title .= '<span class="alert">' . ' - Missing Keys or Out of date you should reinstall!' . '</span>';
          $this->enabled = FALSE;
        }
        $new_version_details = plugin_version_check_for_updates(1292, '2016-01-17 K8');
...
//echo 'MISSING! USPS $type: ' . $type . (in_array($type, $this->typeCheckboxesSelected) ? ' YES' : ' NO') . ' $method: ' . $method . ' $usps_shipping_weight: ' . $usps_shipping_weight . ' $minweight: ' . $minweight . ' $maxweight: ' . $maxweight . '<br>';
  ...
  function install() {
    global $db;
    $db->Execute("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('USPS Version Date', 'MODULE_SHIPPING_USPS_VERSION', '2016-01-17', 'You have installed:', '6', '0', 'zen_cfg_select_option(array(\'2016-01-17\'), ', now())");
...
// disable request for First-Class MailRM Letter at > .21875
          if (($shipping_weight > 1 && MODULE_SHIPPING_USPS_RATE_TYPE == 'Online') && $shipping_weight > 13/16 || ($shipping_weight > .21875 && $requested_type == 'First-Class Mail Letter')) {
            continue;
          } else {
...
// First-Class Commercial
        ...
        elseif ($requested_type == 'USPS Retail GroundTM') {
          $service = 'PARCEL';
        }
    ...
    if ($time == '' || $this->transitTimeCalculationMode == 'CUSTOM') {
      ...
      /********************* CUSTOM START:  IF YOU HAVE CUSTOM TRANSIT TIMES ENTER THEM HERE ***************/
        ...
        case (preg_match('#Priority MailTM#i', $service)):
            $time = '2 - 3 ' . MODULE_SHIPPING_USPS_TEXT_DAYS;
          break;
        case (preg_match('#USPS Retail GroundTM#i', $service)):
            $time = '4 - 7 ' . MODULE_SHIPPING_USPS_TEXT_DAYS;
          break;
        case (preg_match('#First\-Class#i', $service)):
          $time = '2 - 5 ' . MODULE_SHIPPING_USPS_TEXT_DAYS;
