	...
	<div class="account_history">
        ...
        ?>
    		...
    		<div class="forward change_address">
				<?php echo '<a class="cart-buttons" href="' . zen_href_link(FILENAME_ACCOUNT_HISTORY_INFO, (isset($_GET['page']) ? 'page=' . $_GET['page'] . '&' : '') . 'order_id=' . $history['orders_id'], 'SSL') . '">' . zen_image_button(BUTTON_IMAGE_VIEW_SMALL, BUTTON_VIEW_SMALL_ALT) . '</a>'; ?>
            </div>
		</div>
		<?php
            }
        ...
        <?php if($history_split->display_count(TEXT_DISPLAY_NUMBER_OF_ORDERS)!= NULL) { ?>
            <div class="navSplitPagesResult col-lg-4 col-md-4 col-sm-4 col-xs-12 account-history-nav">
				<?php echo $history_split->display_count(TEXT_DISPLAY_NUMBER_OF_ORDERS); ?>
            </div>
        <?php } ?>
            <div class="buttonRow forward change_address account-history-back col-lg-4 col-md-4 col-sm-4 col-xs-12">
                <?php echo '<a class="cart-buttons" href="' . zen_href_link(FILENAME_ACCOUNT, '', 'SSL') . '">' . zen_image_button(BUTTON_IMAGE_BACK, BUTTON_BACK_ALT) . '</a>'; ?>
            </div>
            <div class="navSplitPagesLinks forward pagination-style col-lg-4 col-md-4 col-sm-4 col-xs-12 account-history-nav">
