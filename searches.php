...
if (strlen($wordSearch) > 0) {
	...
	$sqlProduct = "SELECT " . TABLE_PRODUCTS_DESCRIPTION . ".products_name, " . TABLE_PRODUCTS_DESCRIPTION . ".products_id, " . TABLE_PRODUCTS . ".products_image, " . TABLE_PRODUCTS . ".products_tax_class_id, " . TABLE_PRODUCTS . ".products_price, " . TABLE_PRODUCTS . ".products_status FROM " . TABLE_PRODUCTS_DESCRIPTION . ", " . TABLE_PRODUCTS . "
	  ...
	  while (!$dbProducts->EOF) {
		...
		if (strtolower(substr($prodResult,0,strlen($wordSearch))) == strtolower($wordSearch)){
			$results[] = array(
				...
				'pc'=>"p",
				'pr'=>"",
				'img'=>zen_image(DIR_WS_IMAGES . strip_tags($dbProducts->fields['products_image']), strip_tags($dbProducts->fields['products_name']), SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT)
			);
		}else{
			$resultsAddAfter[] = array(
				'q'=>$prodResult,
